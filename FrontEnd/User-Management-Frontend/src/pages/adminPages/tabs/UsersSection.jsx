...
const UsersSection = () => {
    ...
    const {data,isError,isSuccess} = useGetAllUsersDataQuery(undefined,{skip:!adminToken})
    if(isSuccess){
                    ...
                    <tbody>
                      {users.map((user, index) => (
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div className="w-12 h-12  rounded-lg flex items-center justify-center text-white font-semibold">
                               <img src={user.profileImage ?? null} alt="No profile pic" />
                              </div>
                              <div>
                          ...
                          <td className="px-6 py-4">
                            <Badge className={`${getStatusColor(user.isBlocked)} border`}>{user.isBlocked?'Blocked':'Active'}</Badge>
                          </td>
                          {/* <td className="px-6 py-4">
